# DR-020: A Reuse Check as a Step Z Limb
...
- **The gap** — that a result can be arithmetically correct and evidentially empty, with no shipped check naming it — rests on **n=2**: one in-repo instance found by the DR-019 backlog sweep (2026-08-13) and one external document audited three days later. One is self-application. Two instances three days apart, surfaced by the same maintainer with the same instruments, is a pattern worth naming and **not** a measured recurrence rate.
...
The framework touches circularity in five places. All five are judgment calls; none is mechanical.
...
| `templates/anti-hallucination.md:55` | Step 6 *Human-in-loop anchor* | **A different animal** — *procedural* circularity (an agent verifying its own citation) |
| `README.md:522` (R-4) | Ad-hoc retiering, SUPPORTED → EMERGING | One registry row, after the fact |
...
This is decidable by inspecting the derivation graph. It requires no physics judgment, and in the external instance it caught a real defect that no shipped instrument names: an energy comparison reported as confirmation, where the two quantities had been set equal earlier in order to solve for a third.
...
- (+) The test itself is genuinely mechanical.
...
>
> Deciding whether a number is *offered as confirmation* is a judgment; the test itself is not. Where the trace fires, the evidence tier for that claim is at the floor, and Step Z's usual choice narrows: **downshifting the prose is available, supplying more apparatus is not.** No further measurement can raise a result that its own derivation entails.
>
> *Related but out of scope:* a result may also be empty because the posited entity drops out of the final expression, or because the construction is a change of units. Those are real, and they are **not** decidable by substitution — every defined constant vanishes under reduction to primitives, so "what cancels" is not a test. Judge them by asking how many independent predictions the construction buys per observable it consumes.
...
- **The Reuse test fires on legitimate internal consistency checks.** The scope gate is a judgment call, so this is the most likely false-positive route and the sharpest known weakness.
...
1. **The Cancellation limb was unsound.** "Substitute every definition back and see which symbols survive" flags essentially all of derived physics — Rydberg, Bohr radius, Chandrasekhar mass all vanish under reduction to primitives. A determinate-but-invalid test is worse than an acknowledged judgment call, because it manufactures confident false positives with procedural authority. **Dropped.**
...
<!-- verify: cd "$(git rev-parse --show-toplevel)" && grep -q 'Reuse check' templates/anti-hallucination.md && { echo 'CLAIM REFUTED: DR-020 has been implemented — update this section and the Status field'; exit 1; } || echo 'DR-020 correctly unimplemented: Step Z has no reuse check' -->

<!-- verify: cd "$(git rev-parse --show-toplevel)" && n=$(grep -cE '^\|[^|]*\| `(FORMULA|NUMERICAL|DIMENSION|INCONSISTENT|LABEL|ASSUMPTION|OK)` ' agents/equation-checker.md); echo "equation-checker categories: $n"; [ "$n" -eq 7 ] || { echo 'CLAIM REFUTED: the category table is no longer the seven this DR was written against'; exit 1; } -->

<!-- The probe above matches the category CODE in column 2, not the prose label in
     column 1. Written the other way first, it counted 1 and reported FAIL against
     an unchanged file. -->
