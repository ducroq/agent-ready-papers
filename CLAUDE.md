...
- **agent-ready-papers** (this repo): v2.6.1 (2026-08-13 — `/audit-context` run and remediation: `<!-- placeholder -->` convention documented as a Hard Constraint (~48 paths marked), `templates/CLAUDE.md` gotcha-log path no longer hardcodes a two-deep layout, README adoption-tier table qualified, Paper 1 pin reconciled; also versions the post-v2.6.0 `.gitignore` anchoring and `audits/` doc fix. 3 upstream reference-integrity issues filed. Changes confined to existing artifacts = PATCH.)
...
| Making scope or methodology decisions | `decisions/` — 19 decision records (DR-001 through DR-019). Status as of v2.6.1: **Accepted** — DR-002, 004, 006, 007, 008, 009, 010, 013, 017; **Proposed** — DR-003, 005, 011, 012, 014, 015, 016, 018, 019; **Partially superseded** — DR-001. Only Accepted DRs are binding; read the Status field rather than assuming. DR-018 (notation-consistency checker) stages a draft agent in `extensions/`; DR-019 (Step Z surface scope) carries an explicit EMERGING evidence tier and withholds the recurring obligation it was first drafted to create. <!-- verify: cd "$(git rev-parse --show-toplevel)" && n=$(ls decisions/DR-*.md \| wc -l); echo "decisions/ holds $n DR files"; [ "$n" -eq 19 ] \|\| { echo "CLAIM REFUTED: row says 19"; exit 1; } --> |
