...
| Miscase that section's marker (`**Claims:**`) | **exit 0** — passes | **open**: an unrecognised marker is not a marker, so its whole sub-table is skipped; it is an error only when no sub-table parses at all |
