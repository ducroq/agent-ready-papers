...
- **The mechanization argument for the sub-typing, and the checker that would carry it.** This DR argues the split on expressiveness grounds alone — reviewers already make the distinction and the registry has nowhere to record it. A second argument was not available when the DR was written: `tools/check_registry.py` (added 2026-09-14, three months after the 2026-06-11 survey) already walks the premise graph for acyclicity and tier-monotonicity, so a Dung-style labelling — rebutting attacks targeting the conclusion node, undercutting attacks targeting the inference edge — would extend an existing checker rather than introduce new infrastructure. **This is not an argument for building the labelling.** Paper 1's two ARGUMENT rows (`S3-4`, `S4-4`) both carry Rebuttal prose, but neither references another registry entry: the rebuttals are self-limitations, not attacks, so the attack relation is empty and a labelling would return the whole registry. What it argues is that the field's *shape* should stay forward-compatible with an attack graph — which the fourth *Revisit If* bullet already commits to from the reconciliation side. Keep the two consistent if either moves. Whether Rebuttal fields ever come to hold inter-entry IDs rather than prose limitations is the prior question, and is itself a field-test observable.
