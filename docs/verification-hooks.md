...
| Edit | Result | Why |
|------|--------|-----|
| Reclassify the claim P0 → P2 | **exit 1** — still fails | `meets_targets` checks *every* bucket; here P2 had no slack, so the failure moved rather than hid |
| Reclassify the claim P0 → P1 | **exit 0** — passes | **open**: P1 sat at 9/9 against a 90% target, and 9/10 is exactly 90%. Re-tiering hides the failure whenever the destination bucket has slack; no parser can tell a legitimate reclassification from this one |
| Delete the row | **exit 0** — passes | the claim leaves the denominator |
| Blank the Priority cell (delete two characters; the row stays, still marked `[ ]`) | **exit 2** — tooling error | closed 2026-09-26: a row with content but no bucket now raises; only a section divider (nothing after its first cell) may lack one. Until then it exited **0**, because rows with no bucket were skipped |
| Blank every cell but the ID (`\| S2-1 \| \| … \|`, or just `\| S2-1 \|`) | **exit 2** — tooling error | closed 2026-09-26: a row whose first cell matches `S#-#` is never treated as a section divider. An ID the pattern does not match (`S2-1 (dup)`) is still read as a divider and skipped |
| Overwrite the Priority cell (`-`, `TBD`, `P9`) | **exit 1** — still fails | closed 2026-09-26: a priority bucket that is not P0/P1/P2 or a configured target fails `meets_targets`. Until then it became its own untargeted bucket and exited **0**; `p0` and `**P0**` now normalise to `P0` |
| Blank the Status cell | **exit 1** — still fails | closed 2026-09-26: a blank Status counts as unverified. Until then the row left the denominator, so blanking it *raised* coverage |
| Miscase that section's marker (`**Claims:**`) | **exit 0** — passes | **open**: an unrecognised marker is not a marker, so its whole sub-table is skipped; it is an error only when no marker in the file is recognised at all |
| Rename the `Priority` column header | **exit 2** — tooling error | closed 2026-09-26: a sub-table marker whose table has no Priority or Status column now raises instead of being skipped. Until then it exited **0**; with *every* header renamed it also emptied the P0 floor, which passed as "NOT evaluated" even on a registry whose floor fails. ⚠️ Renaming the **marker** instead (`**Claims:**`) still drops that one sub-table silently; it is an error only when no marker in the file is recognised at all |

Four measured routes are left open, and `coverage.py` does not close them: deleting the row; reclassifying it into a bucket with slack; blanking a row down to an ID the `S#-#` pattern does not match (`S2-1 (dup)`, `_S2-1_`); and miscasing its sub-table's marker. None is beyond a tool in principle — a gap in sequential IDs, a Priority that changed against the last commit, or a near-miss marker could each be detected — but a single-file parser is the wrong instrument for the first two. The measured routes that damaged the Priority cell of a row with a matching ID — blanking or overwriting it, blanking the row down to its ID, renaming the header — are closed. What remains is largely a review question (`git diff` shows a deleted row, a changed Priority or a changed marker), which is why the Hard Constraint below still matters. Note that the obvious attack — downgrading a tier — is the one that *doesn't* work here; a constraint written only against downgrading would guard the wrong door.
