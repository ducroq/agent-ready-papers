...
**Current release:** v3.0.0 (2026-08-13) — see [`CHANGELOG.md`](CHANGELOG.md). Pin your project with `agent-ready-papers: v3.0.0` in your CLAUDE.md and surface drift at session start; [`templates/CLAUDE.md`](templates/CLAUDE.md) now carries that line for you.
...
The review produces **scored assessments** — not just prose feedback. A weighted rubric — for non-empirical papers, Rubric B (e.g., contribution 15%, argument quality 20%, evidence-language alignment 10%, clarity 10%) gives you a concrete quality signal. If the simulated review scores below your threshold (e.g., 3.5/5.0), address the weaknesses before submitting.
...
Invoke as `python -m tools.coverage <registry.md>` or via Makefile (`make coverage` / `make check-dois`) from the repo root. Known limits (no HTTPS proxy support, sequential HEAD scaling) are documented in [`tools/README.md`](tools/README.md).
